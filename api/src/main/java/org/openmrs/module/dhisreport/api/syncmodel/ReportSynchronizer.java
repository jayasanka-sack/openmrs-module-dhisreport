...
public class ReportSynchronizer {
	...
	public void fullSync() throws UnsupportedEncodingException {
		...
		for (SyncDataElement item : readDE) {
			...
			List<SyncCategoryCombo> categoryCombo = read
					.readCategoryCombo(html);
			...
			for (SyncDataSet sds : datasets) {
				...
				try {
					...
					log
							.error("Sync failed for Dataelement since data element did NOT have any dataset-"
									+ item);
					e.printStackTrace();
					continue;
				}
			...
			String CategoryComboURL = categoryCombo.get(0).getHref().concat(
					".xml");
			html = fetchURL(CategoryComboURL);

			List<SyncCategoryOptionCombo> disaggregations = read
					.readCategoryOptionCombo(html);
