...
public class DHIS2ReportingServiceImpl extends BaseOpenmrsService
	...
	@Override
	public DataValueSet generateReportingReportDefinition(
		...
		org.openmrs.module.reporting.report.definition.ReportDefinition rrd = Context
				.getService(ReportDefinitionService.class).getDefinitionByUuid(
						reportDefinition.getReportingReportId());
		...
		Parameterizable parameterizable = ParameterizableUtil
				.getParameterizable(reportDefinition.getReportingReportId(),
		...
		if (parameterizable != null) {
			...
			if (parameterizable != null
					&& parameterizable.getParameters() != null) {
			...
			try {
				results = (ReportData) ParameterizableUtil
						.evaluateParameterizable(parameterizable,
								evaluationContext);
				Iterator<Entry<String, DataSet>> iterator = results
						.getDataSets().entrySet().iterator();
	...
	public void saveReportTemplates(ReportTemplates rt) {
		...
		for (ReportDefinition rd : reportdef) {
			...
			Set<DataValueTemplate> datavaluetemplate = rd
					.getDataValueTemplates();
