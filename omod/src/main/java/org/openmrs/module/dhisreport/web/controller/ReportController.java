...
public class ReportController {
	...
	public String executeReport(
			...
			@RequestParam(value = "prior", required = true) String prior,
			WebRequest webRequest, HttpServletRequest request) throws Exception {
		DHIS2ReportingService service = Context
				.getService(DHIS2ReportingService.class);
		Period period = null;
		...
		if (freq.equalsIgnoreCase("monthly")) {
			...
			try {
				...
				period = new MonthlyPeriod(new SimpleDateFormat("yyyy-MM-dd")
						.parse(dateStr));
				// System.out.println( "helloooooooooo2=====" + period );
			} catch (ParseException pex) {
				log
						.error(
								"Cannot convert passed string to date... Please check dateFormat",
								pex);
				webRequest
						.setAttribute(WebConstants.OPENMRS_ERROR_ATTR, Context
		...
		if (freq.equalsIgnoreCase("weekly")) {
			try {
				...
				Integer weekvalue = Integer.parseInt(dateStr.substring(dateStr
						.indexOf('W') + 1)) + 1;
				if (weekvalue > 9) {
					weekvalue = weekvalue == 54 ? 53 : weekvalue;
					finalweek = modify_week[0].concat("W"
							+ weekvalue.toString());
				} else {
					finalweek = modify_week[0].concat("W0"
							+ weekvalue.toString());
				}

				period = new WeeklyPeriod(new SimpleDateFormat("yyyy-'W'ww")
						.parse(finalweek));
		...
		for (Location l : locationList) {
			for (LocationAttribute la : l.getActiveAttributes()) {
				if (la.getAttributeType().getName().equals("CODE")) {
					...
					if (!la.getValue().toString().isEmpty()
							&& la.getValue().toString() != null) {
		...
		for (Location l : locationListFinal) {
			...
			DataValueSet dvs = service.evaluateReportDefinition(service
			...
			for (DataValue dv : datavalue) {
				...
				DataElement detrmp = service.getDataElementByCode(dv
						.getDataElement());
			...
			if (destination.equals("post")) {
				AdxImportSummary adxImportSummary = Context.getService(
						DHIS2ReportingService.class).postAdxReport(adxType);
				agrs.setAdxImportSummary(adxImportSummary);
			}
		...
		org.openmrs.module.reporting.report.definition.ReportDefinition rrd = Context
				.getService(ReportDefinitionService.class).getDefinitionByUuid(
						service.getReportDefinition(reportDefinition_id)
								.getReportingReportId());
		...
		if (rrd != null) {
			...
			rq
					.setReportDefinition(new Mapped<org.openmrs.module.reporting.report.definition.ReportDefinition>(
							rrd, param));
			rq.setRenderingMode(new RenderingMode(new DefaultWebRenderer(),
					"Web", null, 100));
			Report report = Context.getService(ReportService.class).runReport(
					rq);
